 ...
 *
 * 各プリセットが実際に見た目を変えられるかは、生成して確かめてある
 * （追加仕様 §6 / §8.3、検証結果は PRODUCT_SPEC.md §77.24）。
 *
 * 「バランス」は削除した。文字くっきりと見分けがつかず、
 * 選ぶ時点で違いを予測できない設定は、選択肢として働かないため。
 *
 * 残る4つは2つの軸で並んでいる。新しく足すなら、この軸で説明できること。
 *
 *   主役をどちらにするか   キャラクター重視 ←→ 文字くっきり
 *   派手か落ち着いているか  おまかせ ←→ 落ち着いた仕上がり
 *
 * 足す文は、程度を表す形容詞ではなく**数えられる形**で書く。
 * 「派手にしすぎず」「両方はっきり」は効かず、
 * 「文字色は1〜2色」「飾りは付けない」は効いた（§77.24）。
 */
export type StylePresetId = 'auto' | 'character' | 'text' | 'calm';
