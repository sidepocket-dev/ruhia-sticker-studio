...
1. 処理速度のため、マスクを長辺 1024px 以下に縮小して解析し、結果の矩形を元解像度へ写像する
2. Union-Find による2パス Connected Component Labeling（8近傍）
3. 微小成分の除去（面積がシート全体の 0.02% 未満）
4. 各成分の bounding box・重心・面積を算出
5. 近接成分の統合：bbox 間距離がセルサイズに比例した閾値以内なら同一グループ候補とする
6. 3 × 3 の想定中心9点を事前分布として使い、セルサイズで正規化した距離コストで各グループを9枠へ割り当てる
7. グループ矩形どうしが一定以上重なる場合は割り当てを棄却し、重心距離のみの厳格割り当てへフォールバックする
8. 各グループに**確信度**を付与し、低いものにだけ警告アイコンを出して手動修正へ誘導する
...
## 77.9 オフライン配布と IndexedDB の制約

**`file://` から開いたページでは Chrome / Safari が IndexedDB を拒否する**（オリジンが opaque 扱いになるため）。
原仕様§51（IndexedDB による永続化）と §54（HTMLをダブルクリックして利用）はこのままでは両立しない。

実装方針：

1. `ProjectStore` をインターフェースとして定義する
2. 起動時に IndexedDB の可用性を**実測**する
3. 利用できない場合は `memory-store` へ自動フォールバックし、次を表示する
   > このモードでは自動保存ができません。作業を中断する前に「プロジェクトを保存」してください。
4. その環境では `PROJECT_PACKAGE.zip` の書き出し／読み込みを保存手段とする
5. オンライン版（https）では従来どおり自動保存が有効
