...
画像はZIPで一括アップロードできるが、**タグだけは1個ずつ**になる。

```text
セット全体で1回   タイトル / 説明文 / クリエイター名 / コピーライト
1個ずつ           タグ（3個まで・任意）
```

セリフを打ち直す必要はない。文字は画像の中に描かれている。

### タグは自由入力ではない

**300種類以上ある一覧から選ぶ。** 検索欄が用意されていないため、
ブラウザのページ内検索で探すのが定石になっている。

そこでアプリは**探すときの手がかりになる言葉**を出し、
それぞれにコピーボタンを付ける。
...
```text
セリフ      トークで打つ言葉とタグが合っているのが本来の形
種類の名前  セリフで見つからないときの、広い言葉
```

番号は提出順（ZIPの `01.png`）と一致させる。ここがずれると、
別のスタンプにタグを付けてしまう。テストで固定している。

### 書かないこと

**「このタグを設定してください」とは書かない。**
ここに出す言葉がLINEのタグ名だとは限らない。
検索の足がかりであって、タグそのものではない（原仕様 §34）。
