...
### 未確定：実行環境による違い（2026-08-28 時点）

**アプリで1枚しか生成できなかったプロンプトを、ブラウザで開いた ChatGPT で
実行したら動いた**という観察がある（Mac アプリ、1回）。

ただし**これは確定していない。** 同じ文面が成功も失敗もすることは
すでに分かっているため（下記）、1回ずつの比較では
「環境の違い」と「ばらつき」を区別できない。

確かめるには、同じ文面を**それぞれの環境で複数回**試す必要がある。

```text
アプリで毎回1枚 / ブラウザで毎回5枚  → 環境の違い。画面で案内する
どちらも結果が割れる                  → ばらつき。案内しない
```

確定するまで、**画面に環境の話を書かない。** 外れていた場合、
使う人を無駄な回り道へ送ることになる。
