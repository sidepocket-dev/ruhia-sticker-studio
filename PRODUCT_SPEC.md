...
### 検証中：実行環境による違い（2026-08-28 時点）

**同じ文面でも、実行環境で結果が変わる可能性がある。**
...
アプリ側が「失敗しかしない」まで偏ってきたため、ばらつきだけでは
説明しにくくなってきた。ただしブラウザ側がまだ1回なので確定はしない。

**採用条件：ブラウザであと数回成功すること。**
満たしたら、まとめて頼む画面に次を足す。

```text
ブラウザで開いた ChatGPT のほうが、5枚とも生成できることが多いです。
アプリで1枚しか作れないときは、ブラウザでお試しください。
```

満たすまでは画面に書かない。外れていた場合、
使う人をアプリとブラウザの往復という回り道へ送ることになる。
