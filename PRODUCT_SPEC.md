...
### 止まったときは、同じ会話のまま続きを頼む

1枚だけで止まったら、続きを頼む文をそのまま送る。

**新しい会話を始めてはいけない。** 直前のシート1を参照できるので、
...
残りのシート2〜5を、今のシート1と同じ雰囲気・デザインで続けて生成して
  → 画風は引き継げた。続けても書いてくれた。
  → だが4シートぶんを縮小して1枚に詰め込んだ
```

わかったのは、**会話の続きでも出力単位は引き継がれない**ということ。
画風やキャラクターは前の絵から引き継がれるのに、
「1シート＝1枚」の約束だけが抜け落ちる。

そのため続きの文にも、まとめて頼むときと同じものをもう一度書く。

```text
【最重要：出力手順】  何枚作るか。1回の生成でまとめるのではない
出力単位             - 画像1 = シート2のみ …
1枚ぶんの条件         3×3グリッド、9種類、正方形、完全透明背景
【禁止事項】          コラージュ／同じ画像内に配置／
                     ステッカーを小さくして1枚に詰め込まない
```

最後の「小さくして詰め込まない」は、実際に起きたことをそのまま禁止したもの。

`buildContinuePrompt(count, done)` が組み立てる。
`done` は**できた枚数**で、残り枚数・対応表・禁止事項がそれに合わせて変わる。
残り1枚のときは、複数枚を分ける説明を出さない（噛み合わないため）。

**この文はアプリが出す。** まとめて頼む画面に、コピーボタンつきで常に出しておく。
ここで詰まると先へ進めないため、起きてから探させない。

**文面を別に配ることはしない。** 使う人はアプリから受け取るので、
どこかへ貼るための文案を用意する必要はない。**伝えたいことは画面に書く。**
