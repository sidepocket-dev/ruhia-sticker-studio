...
**作るだけでは届かない。** 単一HTMLはビルドもテストもしていたが、
公開サイトに置いていなかったため、**利用者は入手できなかった。**
ソースから `npm run build:offline` を叩ける人しか手に入らない状態は、
「インストール不要」を満たしていない。

```text
公開に含める   dist/RUHiA-Sticker-Studio.html
画面のリンク   フッターに「オフライン版をダウンロード」
```

ファイル名は画面のリンクとデプロイの両方に出る。**食い違うと切れる**ため、
テストで固定する。

**オフライン版自身にはこのリンクを出さない。** そのHTMLの隣にファイルは無く、
リンクが必ず切れる。すでに手元にあるものを配る意味もない。
...
全モデル共通の設定として4つ持つ。
候補は5つあったが、生成して確かめて1つ削った（§77.24）。

```text
おまかせ            何も足さない（既定）
キャラクター重視    キャラクターを主役に見せる
文字くっきり        セリフを大きく、文字色はカラフルでよい
落ち着いた仕上がり  文字色1〜2色・飾りなし。仕事の連絡向け
```
...
（追加仕様 §6 / §8.3）。差が出ない設定は整理する。
検証の結果は §77.24 にある。

---

## 77.24 仕上がり設定の検証（追加仕様 §6 / §8.3）

**実装日:** 2026-08-28

生成して確かめた結果。差が出ないものは整理する。

```text
おまかせ            基準。何も足さない
キャラクター重視    ○ 効く。文字を縦組みで横へ逃がし、キャラクターが大きい
文字くっきり        ○ 効く。文字を横組みで上に大きく置く
落ち着いた仕上がり  ○ 効く（書き直したあと）。下記
バランス            × 削除。文字くっきりと見分けがつかない（下記）
```

### 効いたのは「大きさと置き方」で、色ではない

キャラクター重視には色の指定が**ない**のに、結果はカラフルだった。
つまり**このモデルは放っておいても文字を派手に描く。**

そのため文字くっきりの「文字色はカラフルで構いません」の行は、
おそらく何もしていない。差を生んでいるのは「セリフを大きく」のほう。

### 「派手にしすぎず」は効かなかった

```text
文字も全体も派手にしすぎず、落ち着いた印象にしてください。
仕事の連絡でも使いやすい雰囲気にしてください。
```

この指定で生成しても、文字色は6色以上、ハート・星・キラキラ・
太陽・効果線もそのまま出た。**他の設定と見分けがつかない。**

原因はまとめて頼むプロンプトのときと同じ。
**「派手にしすぎず」は数えられない。** どこまでやれば足りるのか決められない。
...
**書き直したら効いた。** 文字色は濃紺1色になり、ハート・星・キラキラ・
効果線はすべて消え、セリフは吹き出しに入った。同じ用途・同じセリフで
比べて、他の設定とはっきり違う見た目になっている。

### わかったこと：数えられない言葉は効かない

3回とも同じ形の失敗だった。

```text
効かない   5枚生成して / 派手にしすぎず / 両方はっきり
効く       画像1 = シート1のみ / 文字色は1〜2色 / 飾りは付けない
```

**モデルが判定できる形で書く。** 程度を表す形容詞は、
どこまでやれば足りるのかモデルが決められないため通らない。

### 「バランス」は削除した

書き直したうえで、同じ用途・同じセリフで文字くっきりと比べた。
飾りの種類も位置もほぼ同じで、**見分けがつかなかった。**

まったく同じではない。文字くっきりは「ございます」を小さく落とし、
主役の語だけを目立たせる。バランスは2行を同じ調子で置き、
キャラクターがわずかに大きい。**だがこの差は、選ぶ時点で予測できない。**

**選ぶ人が違いを予測できない設定は、選択肢として働かない。**
残る4つは2つの軸で並んでいて、選ぶ理由を言葉にできる。

```text
主役をどちらにするか    キャラクター重視 ←→ 文字くっきり
派手か落ち着いているか  おまかせ ←→ 落ち着いた仕上がり
```

バランスはこの間に入るもので、おまかせと役割が重なっていた。

新しく設定を足すときは、**この軸で説明できること**を条件にする。
足す文は数えられる形で書く（上記）。

### 吹き出しが離れていても取り出せる

落ち着いた仕上がりでは、**セリフが吹き出しに入り、キャラクターから離れる。**
1つのスタンプが2つの塊に分かれて見えるため、取り出しが心配だったが、
実機のシートで9個とも正しく取り出せた（2026-08-29）。

近さでまとめる仕組み（§17）が効いている。この見た目は他の設定にはないため、
仕上がり設定を変えたときは取り出しも確認する。

### 比べるときの注意

用途（日常用・ビジネス用）を変えるとセリフの長さが変わり、
文字の組み方も変わる。**仕上がり設定だけを比べるなら用途は固定する。**
シート1の9種類は個数を変えても同じなので、そこで比べるのが早い。

---

## 77.26 まとめて頼むプロンプト：距離をゼロにする（4回目の書き直し）

**実装日:** 2026-09-03

ここは4回失敗した。**失敗の形はすべて違う。**
...
1〜3は**禁止事項を足す**方向で対処してきた。4はそれでは直らなかった。
...
**「画像N」と「シートNの中身」が離れている。** モデルは対応表を読んでも、
遠くにあるセリフと結びつけられなかった。
...
各画像の見出しの直下に、その画像のセリフを置く。
実機で**5枚とも別内容**を生成できた（2026-09-03、ブラウザのChatGPT）。

**禁止事項は増やしていない。** 増やす方向は行き止まりだと分かっている。

`tests/fixtures/batch-prompt-verified.md` を差し替え、
セリフを画像の見出しより後ろへまとめ直していないこともテストで固定した。

### 上下の隙間を明記した

同じ回のテストで、5枚のうち1枚が段くっつきで分割できなかった（§77.25 と同じ症状）。
レイアウト条件が「ステッカー同士の間には」しか言っておらず、
**左右は守られて上下が崩れる**ことが2回続いた。両モードに次を足した。
...
成功2回・失敗4回。**目玉として説明しない。** 既定は1枚ずつのまま。
ただし**貼る回数を減らすことも、このツールが省く手間のひとつ**なので、
モード自体は残して改良する（追加仕様 §1.2）。

---

## 77.27 通しテストが混んでいるときだけ落ちた原因

**実装日:** 2026-09-03

いちばん重い試験（5枚で45個）が、通しで走らせたときだけ落ちた。
3回起きて、そのうち2回は原因を残さずに報告してしまった。

`trace: 'retain-on-failure'` を入れて記録を残したところ、原因が出た。

```text
待ち時間の予算    120秒（枚数から計算）
テスト自体の上限   90秒（既定30秒 × test.slow()）
```

**テストのほうが先に死ぬので、予算を増やしても効かなかった。**
`waitForStickers` の中で `test.setTimeout(budget + 30秒)` を呼び、
予算に合わせて上限も伸ばす。

残りの遅さは**私が作った負荷**だった。ビルドを2本並走させながら
通すと、5枚の解析が120秒でも終わらない（単独なら約2秒）。
**テストと同時にビルドを走らせない。** 負荷なしなら通しで16〜17秒、
2回連続で142件すべて成功する。

---

## 77.25 公開前テストで見つかった失敗例（段と段がくっつく）

**実装日:** 2026-08-31

学校用のシートを1枚生成して読み込んだところ、**9個そろっているのに分割できなかった。**

```text
縦の隙間（列）   0画素 / 0画素        問題なし
横の隙間（行）   6画素 / 108画素      2段目と3段目の間に隙間が無い
```

108画素ぶんの絵が横一直線に途切れず横切っており、どこで切っても
どちらかのスタンプを切断する。原因はプロンプトの
「幅広い完全透明の隙間」が守られなかったこと。

```text
2段目  「むずかしい…」の机が下へ張り出す ／ 足が下へ伸びる
3段目  「いっしょに勉強しよ」「部活いってくる」の文字が上に高い
```

**左右は守られて上下が崩れた。** プロンプトは「ステッカー同士の間」と書いており、
**上下という言葉が入っていない。**

### 案内が原因の場所を間違えていた（不具合）

```text
出ていた文  空いている場所があるようです。9個そろっているか確認してください。
実際        9個そろっている。隙間が無い
```

**数え直しても原因は見つからない。** 探す場所が違う案内になっていた。

そこで失敗理由に `no-gutter` を足し、隙間の有無を確かめてから案内を選ぶ。

```text
段と段の間に、透明な隙間が見つかりませんでした。
上下のスタンプが近すぎるようです。
文字や小物が上下の段にはみ出していないか確認してください。
```

`empty-cell`（本当に絵が足りない）とは別の理由として扱う。
この判定は「行にも列にも切れる線が1本も無いか」で行う。

実物を `tests/fixtures/sheet-touching-rows.png` に置き、
**この理由が返ることをテストで固定した。** 初めての失敗例。

### 手で切り分けるUIについて

原仕様の Phase 4 が想定していた場面が、初めて実際に起きた。
ただし**このシートは作り直せば解決する**ため、まだ作っていない。
同じ失敗が繰り返し起きるようなら実装する。

---

## 77.23 LINEのタグ設定を手伝う機能は削除した

**追加日:** 2026-08-30 ／ **削除日:** 2026-09-03

タグを探す言葉をコピーボタン付きで並べる機能を作り、削除した。
**前提が間違っていた。**

作ったときの根拠は、公式マニュアルと制作者ブログだった。

```text
1スタンプにつき3個まで          公式マニュアル
検索欄がないのでページ内検索で   制作者ブログ
```

**実際のタグ編集画面を見ていなかった。** 見たら違った（2026-09-03）。
...
`おはよ` のスタンプに「おはよう」「起きる」「かわいい」「トリ」「キュート」まで
推薦されていた。**セリフからは出てこない言葉を含めて、LINE側がすでにやっている。**

こちらの機能は出番が無い。残すと、読む必要のない説明が増えるだけ。

### 教訓

**外部サービスの画面の話を、二次情報で判断しない。**
公式マニュアルであっても古くなる（3個 → 9個）。
機能を作る前に、実際の画面を1度見る。
...
**内容でトリムし、1枚ずつ上限まで大きくする（2026-08-30 変更）。**

```text
内容の外接矩形でトリム
  → 安全余白を加算（既定 8px）
  → 1枚ずつ、幅か高さが上限に触れるまで拡大・縮小
  → 幅・高さを偶数へ切り上げ。拡大は倍率1で頭打ち
```

### 余白は 8px のままにする（2026-08-29 判断）

公式ガイドラインには「コンテンツと外枠の間に10pxぐらいの余白が必要」とある。
本アプリは 8px。**差は2pxで、「ぐらい」の範囲とみなして変えない。**

この値は書き出しだけでなく**切り出しにも使っている**。
広げると隣のスタンプへ食い込むため、`safeMarginPx` は
セル幅と隙間の半分でクランプしている（§77.4）。
書き出しのためだけに動かすと、その調整を二重に持つことになる。

**審査で余白を指摘されたら、そのときに書き出し用の余白だけ分けて 10px にする。**
先回りして複雑にしない。

### 結果：8px のままで審査に通った（2026-09-03）

このツールが書き出したZIPで40個セットを申請し、**LINEの審査に通った。**

```text
https://line.me/S/sticker/36311447
```

余白は指摘されなかった。**8px を10pxへ変える必要はない。**
この行は「未確定の判断」ではなく「確認済み」として扱う。

あわせて次も実績として確定した。

```text
1枚ずつ上限まで拡大した寸法（高さ320固定・幅バラバラ）が審査を通る
01.png〜40.png / main.png / tab.png の構成が受理される
背景透過・偶数寸法・1MB以内の扱いが問題にならない
```

---

### なぜ1枚ずつにしたか（2026-08-30）

以前はセット全体で同じ倍率を使い、シートでの大小関係を保っていた。
その理由は「一律キャンバスにすると表示サイズがばらつく」だったが、
**LINEがアップロードした画像をどう表示するかは確かめていなかった。**
確かめないまま既定を決めていた。

公式ガイドラインが言っているのはここまで。

```text
スタンプ画像は 最大370×320。40個のサイズはバラバラでも構わない
```

一方、制作者の間には**上限に近いほうが有利**という経験則がある
（小さいと一覧でぼやける／視認性でリジェクトされる）。公式の記述ではない。

**そこで、表示の仕組みが分からなくても不利にならない側を選ぶ。**

```text
実寸で並ぶ場合    上限いっぱい → キャラクターのピクセルが多い → 大きく見える
枠に合わせる場合  上限いっぱい → 余白が最小 → キャラクターが枠いっぱいに出る
```

透明余白を足して 370×320 に統一する案（原仕様が想定していた選択肢）は**採らない。**
枠が広がるだけでキャラクターは大きくならず、枠に合わせる仕組みだった場合は
かえって小さく見える。**良くて現状維持、悪ければ損をする。**

**失われるもの：** シートでの大小関係。ただし**その大小はポーズの副産物**で、
演出ではない。お辞儀して縮こまった絵が小さくなっていただけ。

**誤解しやすいところ：「全部同じ大きさ」にはならない。**
縦長のキャラクターでは高さが先に頭打ちになるため、揃うのは高さだけ。
幅は絵の形しだいで散らばる（実測 210〜298px）。

**拡大はしない。** 元の解像度を超えて引き伸ばすとぼやけるため、倍率は1で頭打ち。
実シートは1枚1200px前後あるので、実際には全部が縮小の範囲に収まる。
